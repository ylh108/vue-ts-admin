...
export default class uploadimgDrop extends Vue {
    form = {}
    imgsLoading = false
    pictures: any = []
    showIndex: any = ''
    ...
    imageChange (event: MouseEvent, dropFile?: any) {
        const target: any = event.target
        let name: string = !dropFile ? (target.value.substring(
            target.value.lastIndexOf('\\') + 1)) : dropFile[0].name
    ...
    handleDrop (e: DragEvent) {
        e.preventDefault()
        if (this.imgsLoading || !e.dataTransfer) return
        const files = e.dataTransfer.files
        this.imageChange(e, files)
        // e.stopPropagation()
    }
